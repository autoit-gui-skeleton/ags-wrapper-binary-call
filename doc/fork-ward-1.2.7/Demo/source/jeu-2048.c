...
void win1_onExpose (Ez_event *ev)
{
...
void win1_onKeyRelease (Ez_event *ev)
{
    Gui *gui = ez_get_data (ev->win);
    App *app = gui->app;
...
void win1_onTimerNotify (Ez_event *ev)
{
    Gui *gui = ez_get_data (ev->win);
    App *app = gui->app;
...
void win1_event (Ez_event *ev)              /* Called for each event on win1 */
{
    switch (ev->type) {
        case Expose        : win1_onExpose      (ev); break;
        case KeyPress      : win1_onKeyPress    (ev); break;
        case KeyRelease    : win1_onKeyRelease  (ev); break;
        case TimerNotify   : win1_onTimerNotify (ev); break;
    }
}
...
void gui_init (Gui *gui, App *app)
{
    ...
    gui->win1 = ez_window_create (WIN1_W, WIN1_H, "Game 2048", win1_event);
